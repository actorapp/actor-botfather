...
interface HTTPTrait {
    ...
    fun urlGetBytes(url: String, vararg headers: String): ByteArray
}
...
class HTTPTraitImpl : HTTPTrait {
    ...
    override fun urlGetBytes(url: String, vararg headers: String): ByteArray {
        assumeHttp()
        try {
            val get = HttpGet(url)
            for (i in 0..headers.size / 2 - 1) {
            ...
            val res = client!!.execute(get)
            val bytes = res.entity.content.readBytes()
            res.close()
            if (res.statusLine.statusCode >= 200 && res.statusLine.statusCode < 300) {
                return bytes
            }
    ...
    private fun assumeHttp() {
        if (client == null) {
            val CONNECTION_TIMEOUT = 10000// 10 seconds timeout
            val timeoutConfig = RequestConfig.custom()
                    .setConnectionRequestTimeout(CONNECTION_TIMEOUT)
                    .setConnectTimeout(CONNECTION_TIMEOUT)
                    .setSocketTimeout(CONNECTION_TIMEOUT)
                    .build()
            client = HttpClientBuilder.create().setDefaultRequestConfig(timeoutConfig).build()
        }
    }
}
